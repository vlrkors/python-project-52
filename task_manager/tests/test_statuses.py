...
def test_status_delete_success(client):
    ...
    response = client.post(reverse("status_delete", args=[status.id]), follow=True)

    assert response.status_code == 200
    assert not Status.objects.filter(id=status.id).exists()
    messages = [m.message for m in get_messages(response.wsgi_request)]
    assert any("successfully deleted" in msg.lower() or "успешно" in msg.lower() for msg in messages)
...
def test_status_delete_protected(client):
    ...
    messages = [m.message for m in get_messages(response.wsgi_request)]
    assert any("impossible to delete the status" in msg.lower() or "невозможно удалить статус" in msg.lower() for msg in messages)

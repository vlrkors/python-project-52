...
def test_create_status(auth_client):
    ...
    assert any("Статус успешно создан" in str(message) for message in messages)
...
def test_update_status(auth_client):
    ...
    messages = list(get_messages(response.wsgi_request))
    assert any("Статус успешно изменен" in str(message) for message in messages)
...
def test_cannot_delete_status_in_use(auth_client, user):
    ...
    messages = list(get_messages(response.wsgi_request))
    assert any(
        "Невозможно удалить статус" in str(message) for message in messages
    )
